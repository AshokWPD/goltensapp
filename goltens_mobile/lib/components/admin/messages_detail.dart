  ...
  Future<void> fetchMessages(int groupId, bool refresh) async {
    ...
    try {
      var getMessagesResponse = await AdminService.getMessagesOfGroup(
       ...
       var res = await AdminService.getGroup(id: groupId);
     

      // print(
      //     "${getMessagesResponse.data.first.id}ppppppppp${getMessagesResponse.data.first.messUserID}llllllllll");

       setState(() {
         groupdata = res.data;
        // for (var element in getMessagesResponse.data) {
        ...
        body: Stack(
          children: [
            ...
            Positioned(
              ...
              left: 0,
              right: 0,
              child: ChatTextField(onMessageSend: sendMessage,isSingleMessage: true,groupId:groupdata?.id??0),
            )
          ],
        ),
      ),
