...
class _UsersAndSubAdminsState extends State<UsersAndSubAdmins> {
  ...
  void deleteUsers() async {
    try {
      ...
      await AdminService.updateAdminApproved(
        id: selectedUserSubAdmins[0],
        adminApproved: AdminApproved.rejected,
      );


      if (mounted) {
        const snackBar = SnackBar(content: Text('Users Blocked'));
                    ...
                    children: [
                      Expanded(
                        child: CheckboxListTile(
                          title: const Text("Delete entire data of this user"),
                          value: deleteEntireData,
                          onChanged: (val) {
                            setState(() {
                              deleteEntireData = val ?? false;
                            });
                          },
                      ...
                      Tooltip(
                        // This message appears on long press/hover
                        message:
                            "If checked, all messages, feedback, and other data of this user will be deleted permanently.\nIf not checked, only the user will be removed (soft delete) and their data will remain.",
                        child: IconButton(
                          icon: const Icon(Icons.info_outline,
                              color: Colors.blue),
                          onPressed: () {
                            // Show an additional dialog or snackBar if you prefer
                          },
                        ),
                      ),
                    ],
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                deleteUserslist(deleteEntireData);
              },
            ),
          ],
        );
  ...
  void deleteUserslist(bool deleteData) async {
    try {
      await AdminService.deleteUserslist(
          userIds: [selectedUserSubAdmins[0]], deleteData: deleteData);
  ...
  void showSubAdminDialog({
    ...
    void getdepUser(query) async {
    setState(() {
      search = departmentController.dropDownValue!.name.toString();
      currentPage = 1;
    });
        ...
        floatingActionButton: selectedUserSubAdmins.isNotEmpty
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4.0),
              if(selectedUserSubAdmins.length==1)
