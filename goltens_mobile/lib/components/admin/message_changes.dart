...
class _MessageChangesState extends State<MessageChanges> {
  ...
  Future<void> exportPdfFile() async {
    ...
    var messageId = formatDateTime(
      ...
      'yyMM\'SN${message.id}\'',
        ...
        print("${message.id} ${message.content} ${message.createdAt} $data");
  ...
  Widget buildStatusList() {
    ...
    data.sort((a, b) {
      DateTime aTime = a.reads.isNotEmpty ? a.reads[0].readAt : DateTime.now();
      DateTime bTime = b.reads.isNotEmpty ? b.reads[0].readAt : DateTime.now();
