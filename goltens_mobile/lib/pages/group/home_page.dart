    ...
    try {
      ...
      if (mounted) {
        const snackBar = SnackBar(
          content: Text('Cannot Read HR Documents'),
        );
  ...
  Widget build(BuildContext context) {
    ...
    List<PopupMenuItem<String>> popupMenuItems = [
      const PopupMenuItem(
        ...
        child: Row(
          children: [
            Icon(Icons.feedback), // Icon for 'Go To Feedback'
      ...
      const PopupMenuItem(
        value: 'go-to-checklist',
        child: Row(
          children: [
            Icon(Icons.fact_check), // Icon for 'Go To Checklist'
            SizedBox(width: 10), // Adjust spacing between icon and text
      ...
      const PopupMenuItem(
        ...
        child: Row(
          children: [
            Icon(Icons.video_call), // Icon for 'Go To Toolbox Meeting'
            SizedBox(width: 10), // Adjust spacing between icon and text
...
//       if (element.messUserID != '' &&
//           element.messUserID != user!.id.toString()) {
//         unreadMessages.remove(element);
//       }
//     }
          // ignore: avoid_function_literals_in_foreach_calls
          // GetAllGroupsResponseData groupDatas =await 
          //     groups[index].unreadMessages.forEach((element) { })
    ...
    if (otherFiles.isEmpty) {
      return const Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'No Company Informations Available',
              style: TextStyle(fontSize: 16.0),
            ),
          ],
        ),
    ...
    if (userOrientationItems.isEmpty) {
      return const Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              'No HR Documents Files Available',
              style: TextStyle(fontSize: 16.0),
            ),
          ],
        ),
  ...
  Widget buildGroupListItem(GetAllGroupsResponseData group) {
    ...
    // element.messUserID==user!.id.toString()?unreadMessages.remove(element):null;

    // for (var element in group.unreadMessages) {
    //         if (element.messUserID == "")
    //           {
    //             unreadMessages.add(element);
    //             }
    //         else if (element.messUserID == user!.id.toString())
    //           {unreadMessages.add(element);}
    //       }
    //var unreadMessages = group.unreadMessages;
    var time = '';
