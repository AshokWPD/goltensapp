...
class _FeedbackPageState extends State<FeedbackPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      nameController.text = "Name: ${user!.data.name} / Employee No: ${user!.data.employeeNumber} ";
      ...
      setState(() {
        ...
        locationController.text = feedback?.location ?? "";
  ...
  Future<File> exportPdfFileMail(FeedbackData feedback) async {
    List<Uint8List?> bytesArray = [];
    List<Uint8List?> actionBytesArray = [];
  ...
  Future<void> shareSubmitResponse(FeedbackData feedback) async {
    File file = await exportPdfFileMail(feedback);

    final Email email = Email(
      body: 'Submitted Feedback Form Of SERIAL ID: ${feedback.id} (PDF)',
      subject: '',
      recipients: ['poovan.alves@goltens.com'],
  ...
  Widget build(BuildContext context) {
    ...
    // Future<void> exportAcknowledgePdfFile() async {
    //   final ByteData image = await rootBundle.load('assets/images/logo.png');
    //   Uint8List logoImage = (image).buffer.asUint8List();
    //
    //   final directory = await getDownloadsDirectoryPath();
    //   Uint8List pdfInBytes = await PDFGeneratorDC.AcknowlendgementPDF(feedback,logoImage);
    //
    //   File file = File(
    //     '$directory/Acknowledgement-receipt -${feedback!.id}.pdf',
    //   );
    //
    ...
    Future<void> exportAcknowledgePdfFile() async {
      ...
      // Save the PDF file
      if (feedback != null) {
        Uint8List pdfInBytes = await PDFGenerator.generateAcknowledgeFeedbackDetail(
          feedback?.id ?? 0,
          feedback?.createdBy.name ?? '',
          feedback?.createdBy.email ?? '',
          feedback?.createdBy.phone ?? '',
        ...
        File file = File('$directory/Acknowledged-feedback-data-${feedback?.id}.pdf');
        file.writeAsBytes(pdfInBytes);
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          if (isprocess) const Center(child: CircularProgressIndicator()),
                    ...
                    child: Text(
                      (() {
                        switch (selectedStatus) {
                          ...
                          case Status.closed:
                            return "✔ Closed";
                            case Status.closedWithoutAction:
                                  ...
                                  .map(
                                    (obj) => ListTile(
                                      leading: CircleAvatar(
                                        child:
                                            obj.user.avatar.isNotEmpty == true
                                                ? ClipRRect(
                                                    borderRadius:
                          ...
                          children: [
                            ...
                            ClipOval(
                              ...
                              child: Container(
                                width: 40,
                                height: 40,
                                color: feedback?.acknowledgement != null?primaryColor:Colors.grey.shade500,
                                  child: IconButton(onPressed:(){
                                    feedback?.acknowledgement != null? exportAcknowledgePdfFile():null;
                                  }  , icon: Icon(Icons.download_for_offline_rounded,color: Colors.black,))),
                            )
                          ],
                        ),
                        const SizedBox(height: 20),
