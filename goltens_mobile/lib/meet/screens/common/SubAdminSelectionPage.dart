...
class SubAdminSelectionPage extends StatefulWidget {
  ...
  @override
  State<SubAdminSelectionPage> createState() => _SubAdminSelectionPageState();
}

class _SubAdminSelectionPageState extends State<SubAdminSelectionPage> {
  int currentPage = 1;
  int totalPages = 1;
  int limit = 1000000;
...
// Inside the buildBody method
  ...
  void sendNotifications() async {
    ...
    await apiService.sendNotification(
        selectedUsers,
        widget.title,
        "Location: ${widget.location} & Department:${widget.Dep}  @ ${widget.date} as ${widget.type}",
        "meeting_join",
        "",
        "",
        widget.title,
        "Location: ${widget.location} & Department: ${widget.Dep} ",
        "",
        "${widget.date} as ${widget.type}");
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Meet Assigned Successfully...'),
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text(
              search?.isNotEmpty == true ? 'Results for "$search"' : 'User',
  ...
  void filterByDepartment(String? department) {
    ...
    fetchUsersAndSubAdmins(); // Fetch users based on the new filter
  }
}
